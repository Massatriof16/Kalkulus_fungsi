...
/********************************************************************
 * UTILITY UNTUK FILE BINER (HEX CONVERSION)
 ********************************************************************/
...
/********************************************************************
 * ENKRIPSI & DEKRIPSI BYTES (FILE) - DIMODIFIKASI UNTUK MENGGUNAKAN HEX
 ********************************************************************/
function encryptBytes(bytes, b) {
    // Konversi bytes ke hex string, lalu enkripsi sebagai teks
    const hex = bytesToHex(bytes);
    return encryptText(hex, b);
}

function decryptBytes(base64, b) {
    try {
        // Dekripsi sebagai teks untuk mendapatkan hex string
        const hex = decryptText(base64, b);
        if (hex.startsWith("Error:")) return null;
...
/********************************************************************
 * ENKRIPSI FILE - DIMODIFIKASI UNTUK MENGGUNAKAN HEX
 ********************************************************************/
document.getElementById("encrypt-file").addEventListener("click", () => {
    ...
    reader.onload = e => {
        ...
        // Step 2 — Encrypt (menggunakan hex conversion)
        const base64 = encryptBytes(bytes, pin);
        const encrypted = customEncode(base64);

        fill.style.width = "80%";
        status.textContent = "📦 Menyiapkan file...";

        // Step 3 — Export sebagai text file
...
/********************************************************************
 * DEKRIPSI FILE - DIMODIFIKASI UNTUK MENGGUNAKAN HEX DAN MIME TYPE
 ********************************************************************/
document.getElementById("decrypt-file").addEventListener("click", () => {
    ...
    reader.onload = e => {
        ...
        const customText = e.target.result;
        const base64text = customDecode(customText);
        ...
        // Step 2 — Convert to Blob dengan MIME type yang benar
...
// SEND FILE - DIMODIFIKASI UNTUK MENGGUNAKAN HEX
document.getElementById("send-file").addEventListener("click", async () => {
    ...
    reader.onload = async e => {
        ...
        // Kirim sebagai text/plain (Base64 terenkripsi)
        const blob = new Blob([encrypted], { type: "text/plain" });
